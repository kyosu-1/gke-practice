...
├── go/
│   └── services/
│       └── echo/               # 1つ目のサービス（将来 go/services/worker/ 等を追加）
│           ├── main.go
│           ├── main_test.go
│           ├── go.mod
│           └── Dockerfile
...
| GET | /metrics | Prometheus format | Phase 2 監視用（未実装） |
...
|---|---|---|
| Sync Policy | 自動 | 自動 |
| Source Path | k8s/services/echo/overlays/dev | k8s/services/echo/overlays/prod |
| Self-heal | 有効 | 有効 |
| Prune | 有効 | 無効 |
